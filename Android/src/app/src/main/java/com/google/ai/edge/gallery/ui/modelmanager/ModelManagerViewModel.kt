...
constructor(
  ...
  // ANVI: Download functionality removed - uses local models only
  fun downloadModel(task: Task, model: Model) {
    // No-op: ANVI uses local models only
  }
  ...
  // ANVI: OAuth authentication removed - uses local models only
  fun getAuthorizationRequest(): AuthorizationRequest {
    throw UnsupportedOperationException("OAuth not supported in ANVI")
  }
  ...
  fun handleAuthResult(result: ActivityResult, onTokenRequested: (TokenRequestResult) -> Unit) {
    ...
    onTokenRequested(
      TokenRequestResult(
        ...
        errorMessage = "OAuth not supported in ANVI",
      )
    )
  }
  ...
  fun getGeminiApiKey(): String {
    return dataStoreRepository.readGeminiApiKey()
  }

  fun saveGeminiApiKey(apiKey: String) {
    dataStoreRepository.saveGeminiApiKey(apiKey)
  }
  ...
  // ANVI: No model allowlist loading needed - uses fixed local model only
  fun loadModelAllowlist() {
    // Process tasks immediately without loading allowlist
    val curTasks = customTasks.map { it.task }
    processTasks()
    _uiState.update { createUiState().copy(tasks = curTasks) }
  }
