...
abstract class ChatViewModel() : ViewModel() {
      ...
      if (lastMessage is ChatMessageText) {
        ...
        val newLastMessage =
          ChatMessageText(
            ...
            modelType = modelType.ifEmpty { lastMessage.modelType },
          )
        newMessages.removeAt(newMessages.size - 1)
  ...
  fun updateLastTextMessageLlmBenchmarkResult(
    ...
    if (newMessages.size > 0) {
      val lastMessage = newMessages.last()
      if (lastMessage is ChatMessageText) {
        // Preserve all fields when updating benchmark result
        val updatedMessage = ChatMessageText(
          content = lastMessage.content,
          side = lastMessage.side,
          latencyMs = lastMessage.latencyMs,
          accelerator = lastMessage.accelerator,
          isMarkdown = lastMessage.isMarkdown,
          llmBenchmarkResult = llmBenchmarkResult,
          data = lastMessage.data,
          modelType = lastMessage.modelType,
        )
        newMessages.removeAt(newMessages.size - 1)
